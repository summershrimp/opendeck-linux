...
static void goodix_config_cb(const struct firmware *cfg, void *ctx)
{
	...
	if (cfg) {
		...
		u8 check_sum = 0;
		int i;
		char *write2gt911 = gt911_800_480;
		write2gt911[GOODIX_CONFIG_911_LENGTH - 1] = 0x01; // update flag
		for (i = 0; i < (GOODIX_CONFIG_911_LENGTH - 2); i++)  // calc checksum
			check_sum += write2gt911[i];
		write2gt911[184] = (~check_sum) + 1; //checksum

		error = goodix_i2c_write(ts->client, GOODIX_REG_CONFIG_DATA, write2gt911, GOODIX_CONFIG_911_LENGTH);

		dev_warn(&ts->client->dev,
				"using the driver defined config \n");
		/* Let the firmware reconfigure itself, so sleep for 10ms */
		usleep_range(10000, 11000);
	}
